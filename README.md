...
Legacy canonical releases created before `ingest-receipt.json` existed can be migrated directly from Staging. When such a release is selected, the target inspection builds a read-only baseline from its current canonical audio masters, video masters, and authored artwork, hashes every baseline copy, and exposes **Migrate legacy receipt** before the normal update preview. The migration is fingerprint-guarded: if canonical bytes change after review, apply is refused. A successful migration writes only the previously missing receipt; media/TOML remain untouched. Artwork replacement still requires the normal explicit occupied-target confirmation afterward.
...
HLS-stream/waveform/browser-artwork preparation is the first write-enabled Publish action and writes only reproducible derivatives inside the private canonical release. **Make Public** is now the second write-enabled action: it reconstructs the exact reviewed sanitized package in an isolated sibling operation directory, verifies metadata/source fingerprints, copies only planned browser artwork, waveforms, and HLS files, generates `release.json`, per-track `track.json`, `publication-manifest.json`, and `catalog.json`, rejects unplanned/private files, verifies hashes, and atomically promotes the release and catalog. If the release is already public, the same guarded operation is labeled **Update Web Package** and backs up the previous release/catalog for rollback before replacement.

Published Media Fleet & Deployment Bundle v1 adds a whole-catalog deployment gate after release-scoped publication. The Web Package workspace verifies every directory under `published-media/releases/` against its `publication-manifest.json` and the root `catalog.json`, reports orphan directories or unexpected root files as blockers, ignores standard operating-system/editor junk such as `.DS_Store`, `Thumbs.db`, AppleDouble `._*`, `__MACOSX`, editor backup `*~`, and Vim swap files at any depth and excludes them from the deployment snapshot, and summarizes current, update-available, not-published, blocked, and preparation-needed Library releases. Package writes and deployment-manifest refreshes also prune that ignored junk from the local `published-media` tree so the sanitized Web Package itself converges instead of merely hiding junk from deployment. A verified tree can create `published-media/deployment-manifest.json`, which records the catalog hash, every manifest-controlled public file hash/size, each release publication-manifest identity, package-contract versions, and one snapshot content fingerprint. The deployment manifest intentionally excludes itself from that fingerprint. A missing or stale deployment manifest means the tree is not yet deployment-ready even when every individual release is valid.
...
`verify:published-media` is read-only and exits non-zero for integrity blockers, an empty public tree, a missing/stale deployment manifest, or public permission drift. The deployment audit requires every `published-media/` file to be `0644` and every directory to be `0755`; `public-file-mode-invalid` and `public-directory-mode-invalid` are blocking issues and planning never silently chmods the reviewed source package. `audit:public-v1` is the stricter Hiplingo v1.0.0 release gate: it reports a public-package size breakdown and blocks on pending Included-release updates, published-only releases, public video files/video-enabled public releases, legacy `waveform-peaks.json`, non-v6 release contracts, compact-waveform count mismatches, stale deployment-manifest state, or physically present OS/editor junk. `manifest:published-media` prunes ignored OS/editor junk and writes only the deployment manifest after the complete public tree verifies. `stage:published-media` requires an explicit confirmation token, refuses to overwrite an existing target, requires a current deployment manifest, and copies only the verified sanitized snapshot into a new local directory. It does not SSH, rsync, configure nginx, or write to the public server.
...
Library release summaries expose the total audio run time in Rows, Cards, Tiles, and Waveform view, and the selected release row in the metadata sidebar repeats that total. The value is derived from probe-readable canonical audio masters; it is omitted when a complete total cannot be established. The persistent footer player remains track-focused and never displays release-total run time.
...
Waveform
  waveform-peaks.wfp  schema v2, 400 peaks/second, 1024-point Hann FFT
```
...
Publish contract v6 plans a host-ready audio/video layout without deployment-domain URLs:
...
- Library Waveform renders the same shared `MediaVisualizationSurface` as Hiplingo: fixed-center audible scrubbing, the 2–6400 px/s zoom ladder, transition past maximum waveform zoom into the 2048–128-sample oscilloscope, press-and-hold freeze inspection, cached per-track frames, and shared zoom chrome. metadata-editor continues to supply its private playback/master source while Library Rows, Cards, Tiles, Release overview pages, and the footer/player own track selection.
...
Library Artists are first-class private entities stored once under `media-library/artists/<slug>/artist.toml`. A release may reference its canonical primary Artist with `release.primary_artist.id`; the existing credited `name` and `sort_name` remain intact for metadata/export compatibility. Artist artwork is Artist-scoped and must never be inferred from or replaced by release cover artwork.

The initial four-Artist/13-release migration is deliberately separate from application startup. `npm run migrate:artists` prints a read-only deterministic plan. Applying it requires `npm run migrate:artists -- --apply --confirm MIGRATE_ARTISTS_V1`; the writer refuses fuzzy artist-name matches, creates canonical Artist records, backs up each changed `release.toml` under that release's `.metadata-backups/`, and adds only the stable Artist ID association.
...
Artist Web Package membership is derived from the release public set rather than maintained independently. Each Included release contributes its stable `release.primary_artist.id`; metadata-editor prepares only the canonical Artist entities required by those Included releases. A Published-only release can keep its Artist requirement only when its sanitized `release.json` already carries that explicit stable ID; metadata-editor never guesses an Artist from a display name. Adding another Included release by an already-required Artist does not force a redundant Artist snapshot rebuild, while removing the final Included release for an Artist removes that Artist from the next complete Artist snapshot. The Web Package release view opens on **All Library** and gives every release a persistent **Public** checkbox showing its current membership. Activating that checkbox opens the existing Ready Check; the reviewed Add/Remove action remains the only way membership changes. The release Ready Check remains the guarded place to add or remove release membership; batch **Media prep** checkboxes only create private derivatives and never change the public set. Standalone Artist publication without an Included release remains a future explicit workflow.
...
Its associated releases reuse the same filtered Library release browser used by
the main Releases scope, including artwork-first Tiles, Rows, Cards, Waveform,
and sorting controls. Opening a release from either the main Library or an Artist-filtered Library
now enters a browsing-oriented Release overview with artwork, public description,
release facts, track listing, and playback. The deep metadata editor is an
explicit secondary action behind Edit metadata; Ingest/Staging may still open
that editor directly as part of authoring work. Returning to Library restores the
Artist/browser context rather than forcing the main release list. Main Releases
and Artist-filtered releases keep independent in-session display modes so one
scope cannot unexpectedly change the other. Release Description is a registered
multiline field under Text and Notes in the release metadata editor.
...
## Library Waveform follows Now Playing

Library Waveform is a visualization of the persistent application's current
track, not a second release/track browser. Rows, Cards, Tiles, Artist-filtered
Library views, and Release overview pages own track discovery and selection.
Waveform follows that selection automatically and therefore cannot display a
different track from the persistent footer player.

The shared Hiplingo waveform/oscilloscope surface and the exact 45 ms audible
scrub audition contract remain unchanged.

## Shared listener metadata preview

The persistent metadata-editor player exposes a Track information (`i`) action
for Library-backed tracks. It adapts private Library TOML documents into the
same listener-facing metadata presentation consumed by hiplingo.com through
