...
- Staging Artwork & files uses a compact thumbnail-only palette plus release/track destination rows for visual front-artwork assignment. The palette deliberately omits filenames, paths, status badges, assignment badges, and action buttons; thumbnails can be dragged to a destination or selected by click/keyboard for the non-drag fallback. Source review and detach/remove controls live with the collapsed advanced artwork editor, which repeats a compact thumbnail beside each source's role controls and uses source-level dividers and spacing so alternate and secondary-role assignments remain visually identifiable. Scope and role changes are applied immediately with a success toast; Add another assignment creates a new assignment row rather than saving the current one. Folder-derived suggestions still appear on their inferred release or numbered track rows automatically, and TIFF/TIF previews are generated in memory through FFmpeg without modifying the ingest source. During an existing-release update, current canonical Library artwork is shown on the same destination rows, existing tracks remain targetable even when no audio from those tracks is in the new candidate, new artwork roles can be added, and replacing an occupied front-artwork target requires an explicit confirmation before the verified Library copy and its TOML reference may be superseded.
