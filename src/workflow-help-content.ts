...
export const workflowStages: readonly WorkflowStage[] = [
...

  {
    ...
    currentNote:
      "The Live page is a first-class workspace for remote comparison. It defaults inspection to the named production profile using the built-in SSH alias boundary `ssh:hiplingo-prod:/var/www/hiplingo.com/published-media`, while production writes remain disabled in the browser. Check Live is an explicit checksum-based read-only comparison; merely opening Live or resolving the production profile does not contact or modify the remote server. The existing guarded CLI lifecycle remains the only production-write path. Credentials, private keys, server IPs, and raw SSH configuration remain outside metadata-editor.",
  },
] as const;
