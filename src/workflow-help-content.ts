...
export const workflowFaqItems: readonly WorkflowFaqItem[] = [
  ...
  },
  {
    ...
    answer:
      "The Library edit first makes the already-published release Update available. Run Update public package for that release: publication rebuilds the complete release snapshot as a unit, so public files for the removed track cannot survive the replacement. Then Verify snapshot, Refresh deployment manifest, Check host, and deploy the reviewed removal plan. Verify snapshot and Refresh deployment manifest alone never copy Library edits into published-media.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "Deployment verification answers whether the current published-media snapshot is internally complete and hash-consistent. Library freshness answers whether already-published releases still match their current canonical Library inputs. If a published release is Update available, use Update public package first. Verify snapshot and Refresh deployment manifest do not copy Library edits into published-media. Local-sandbox and CLI deployment reject pending published-release changes by default; intentionally deploying the older public snapshot requires an explicit override. Releases that have never been published remain deliberately outside the snapshot and do not block deployment.",
  },
  {
