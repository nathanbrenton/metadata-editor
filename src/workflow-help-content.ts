...
export const workflowStages: readonly WorkflowStage[] = [
...

  {
    ...
    currentNote:
      "The Live page is a first-class workspace for remote comparison. Phase 1 defaults inspection to the named production profile while production writes remain disabled in the browser; the existing guarded CLI lifecycle remains the only production-write path until explicit production activation. The intended target uses the SSH alias boundary only: ssh:hiplingo-prod:/var/www/hiplingo.com/published-media. Credentials, private keys, server IPs, and raw SSH configuration remain outside metadata-editor.",
  },
] as const;
...
export const workflowFaqItems: readonly WorkflowFaqItem[] = [
  ...
  },
  {
    ...
    answer:
      "Use Web Package → Prepare for Web after Ready Check passes and web derivatives are current. The writer copies only the reviewed player-facing resources into a staged snapshot, validates them, and atomically promotes the result; it never moves the canonical release. Later metadata, artwork, or revised-master changes remain authored in media-library and are republished with Update Web Package.",
  },
  {
    question: "How do I remove a release from the public catalog without deleting the Library release?",
    answer:
      "Open Web Package Ready Check for a release that already has a public package and choose Review unpublish. Metadata Editor first fingerprints the current public release tree and catalog membership without writing anything. After confirmation, guarded unpublish removes only that sanitized release directory and its catalog.json entry, verifies the public removal, and journals the operation for interrupted-operation recovery. The canonical media-library release, masters, TOML, prepared private derivatives, and ingest history remain untouched. Refresh the deployment manifest afterward, then Check Live so sandbox or production deployment can propagate the removal.",
  },
  {
    question: "What does Published-only mean in the Publish fleet?",
    answer:
      "Published-only means catalog.json still contains a public release that is not present in the current canonical Library scan. Metadata Editor never interprets that mismatch as permission to delete public content. The fleet surfaces it explicitly for review; use Review unpublish only when you intentionally want that release to leave the public catalog. This also provides a guarded cleanup path if a canonical release was archived or removed from the active Library separately.",
  },
  {
    question: "If I remove a track from a Library release, how does that disappear from the website?",
    answer:
      "The Library edit first makes the already-published release Update available. Run Update Web Package for that release: publication rebuilds the complete release snapshot as a unit, so public files for the removed track cannot survive the replacement. Then Refresh status, Refresh deployment manifest, Check Live, and deploy the reviewed removal plan. Refresh status and Refresh deployment manifest alone never copy Library edits into published-media.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "Hover over any workflow tab to see its configured runtime path and purpose. Ingest reads the disposable source drop without modifying it. After a reviewed build succeeds, that ingest candidate can be deleted. Staging creates or updates releases directly in the private canonical media-library root, and Library authors that same long-term source of truth. Web Package Ready Check may prepare reproducible stream/index.m3u8 + fMP4 segment assets and waveform-peaks.json inside that private release after explicit review. Build or Update Web Package then creates the sanitized snapshot in the configured published-media root from current metadata, browser artwork, waveforms, and HLS resources. Canonical masters, TOML, ingest receipts, stream-info.json, private playback MP3s, production notes, and editor-only sample-clearance administration are never copied into the public release.",
  },
  {
    question: "Why can a verified deployment snapshot still say Library changes pending?",
    answer:
      "Deployment verification answers whether the current published-media snapshot is internally complete and hash-consistent. Library freshness answers whether already-published releases still match their current canonical Library inputs. If a published release is Update available, use Update Web Package first. Refresh status and Refresh deployment manifest do not copy Library edits into published-media. Local-sandbox and CLI deployment reject pending published-release changes by default; intentionally deploying the older public snapshot requires an explicit override. Releases that have never been published remain deliberately outside the snapshot and do not block deployment.",
  },
  {
