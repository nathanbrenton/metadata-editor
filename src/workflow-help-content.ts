...
export const workflowStages: readonly WorkflowStage[] = [
  {
    ...
    currentNote:
      "Read-only candidate scanning, row-based inspection, first-class probe-verified video detection, selectable release-identity sources, source-path/folder-structure evidence, ffprobe/MediaInfo evidence, recognized FFmetadata sidecar evidence, artwork preview, rescanning, and locally saved draft attachment state are available now. Probe-verified video sources can continue to Staging, where each included video receives a reviewed stable ID, descriptive type, canonical video-master destination, and optional semantic relationship to one track while remaining release-scoped. FFmetadata sidecars can be introduced with a new release or later as revision evidence for an existing Library release; paired values seed unambiguous new-release identity/track drafts, while existing authored Library metadata is compared without silent overwrite. Structural suggestions remain editable in Staging and ambiguous multi-audio, multi-image, or metadata-source conflicts are surfaced for review.",
  },
  {
    id: "staging",
    title: "Staging",
  ...
  },
  {
    ...
    steps: [
      ...
      "Release cards include canonical video counts and a compact Videos disclosure when video assets exist. Each row shows the video title/stable ID, descriptive type, release-level or related-track relationship, and whether exactly one canonical video master was detected. Browser-direct masters can be previewed read-only, and Edit metadata can update the canonical title, flexible type, or related-track relationship without changing the stable video ID or canonical master path. Developer/Admin Tools can inspect the underlying video-master files. Library scanning reports missing video.toml, missing/duplicate video masters, and broken related-track references without treating video as an audio track.",
  ...
  },
  {
    ...
    currentNote:
      "The Publish tab starts with a compact five-column readiness overview: Release, Sources, Public media, Status, and Next step. Each release row includes a release-artwork thumbnail and one primary Continue to preflight button. After preflight, the default view is deliberately concise: one plain-language result, separate Web stream and Waveform readiness, and one clearly labeled next action—Resolve blockers, Prepare release, Publish public package, or Update public package. Problems/warnings, the itemized package plan, and the technical contract/fingerprint remain collapsed until opened. Preflight itself is read-only. Prepare release generates the website-listening derivative as AAC-LC HLS with roughly three-second fMP4 segments plus independent waveform-peaks.json data; it never exposes the canonical master or requires the private audio-playback.mp3 to enter the website package. The HLS playlist uses relative segment references so future private storage/CDN authorization can be added without cloud-provider coupling. Publish public package stages only sanitized JSON metadata, browser-compatible release/track artwork, current waveform data, and current HLS files; it validates the complete staged tree and resource hashes before atomically promoting it under published-media and updating catalog.json. Republish uses the same operation as Update public package: the previous public release/catalog are backed up, the new snapshot replaces the release as a unit so obsolete files cannot survive, and a failed promotion rolls back. Publish records a stable content fingerprint in publication-manifest.json and compares it with the current canonical metadata/public-media inputs on later preflight runs. When they match, the UI reports Public package is up to date and removes the repeat publish action; a canonical metadata or public-media change changes that state to Update available. While Prepare release is running, Publish shows live server-reported progress for the active track and phase—Library playback MP3, HLS transcode, waveform generation, validation, and promotion—without flooding the interface with per-track toasts. Successful Prepare and Publish/Update writes use the application's transient success toast while the persistent preflight state remains the source of truth. The CLI commands publish:plan and preflight:publish remain read-only.",
  },
] as const;
...
export const workflowFaqItems: readonly WorkflowFaqItem[] = [
  ...
  },
  {
    ...
    answer:
      "Ingest uses ffprobe to verify real video streams, then Staging → Videos lets you review inclusion, title, a flexible descriptive type, stable video ID, and an optional related track. Included source bytes are copied unchanged to videos/<stable-id>/video-master.<original-extension> and a sibling video.toml records canonical identity and the optional track relationship. The video remains release-scoped even when related to a track. Existing verified Library videos omitted from a later ingest candidate are preserved automatically. The Library scanner recognizes those canonical video directories and the Library release card exposes video identity, type, relationship, and master readiness. Browser-direct MP4, M4V, MOV, and WebM masters can be previewed read-only from the Library without generating a derivative; codec support still depends on the browser. Edit metadata updates the canonical video title, flexible type, and optional related-track relationship in video.toml using a SHA-256 concurrency check, timestamped backup, validated temporary write, and atomic replacement. Stable video ID and canonical master path stay read-only because identity/master replacement require separate reviewed workflows. Live preview transcoding, poster generation, canonical-video replacement, richer video metadata fields, and public web-video derivatives remain later milestones.",
  },
  {
