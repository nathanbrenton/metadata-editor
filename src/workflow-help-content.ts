...
export const workflowStages: readonly WorkflowStage[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      "Release cards include canonical video counts and a compact Videos disclosure when video assets exist. Each row shows the video title/stable ID, descriptive type, release-level or related-track relationship, and whether exactly one canonical video master was detected. Browser-direct masters can be previewed read-only, and Edit metadata can update title, flexible type, description, date, location, director, camera operator, or related-track relationship without changing the stable video ID or canonical master path. Developer/Admin Tools can inspect the underlying video-master files. Library scanning reports missing video.toml, missing/duplicate video masters, and broken related-track references without treating video as an audio track. The private web-video derivative uses one deterministic H.264 High 4.1/yuv420p + AAC-LC HLS rendition that fits within 1280×720 without upscaling, preserves source frame rate up to 60 fps, uses bounded CRF encoding with libx264, includes 192 kbps stereo AAC-LC when source audio is present, aligns keyframes to three-second fMP4 segments, and generates a deterministic PNG poster frame from the first 120 decoded frames. Source/profile fingerprints make both stream and poster freshness reproducible. A guarded backend plans and prepares this private video media through staging, HLS/decode validation, stale-plan rechecking, SHA-256 verification, atomic promotion, and rollback. Publish preflight reports video readiness separately, Prepare video media generates missing or stale private HLS + poster resources, and public-package contract v4 publishes sanitized video.json, poster.png, and current segmented HLS resources; canonical video-master files, video.toml, and private stream-info.json remain excluded.",
...
export const workflowFaqItems: readonly WorkflowFaqItem[] = [
  {
    ...
    answer:
      "Use the Hiplingo wordmark at the far left of the application header. It is the global home control and always returns to the Library landing workspace. The contextual Back to editor control inside Workflow & Help still returns to the workspace or release detail that opened Help.",
  },
  {
