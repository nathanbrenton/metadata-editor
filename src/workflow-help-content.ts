...
export const workflowStages: readonly WorkflowStage[] = [
...

  {
    ...
    currentNote:
      "The Live page is a first-class deployment-destination workspace. Its release list mirrors the exact Web Package → Included set, preserving the same release artwork, release dates, and sort order while emphasizing the selected target state beside each included release. Not included Library releases are never sent to either target; if a checked target still contains a release absent from the current Included set, the comparison surfaces it in a separate release-level Leaving Live warning with Review before deploy. File-level removal counts stay in Connection & deployment details. Live opens on the named Local sandbox profile so the normal metadata iteration loop is Web Package → Check Local sandbox → Deploy to sandbox without risking SSH. The always-visible destination control switches explicitly between Local sandbox and Production. Production uses the built-in SSH alias boundary `ssh:hiplingo-prod:/var/www/hiplingo.com/published-media`; Check Production is read-only and production writes remain disabled in the browser. Merely opening Live or selecting a profile does not modify either target. Live always shows the total verified Web Package size. When the comparison is a complete add-only first deployment, the UI also shows that package size as the approximate upload size; mixed incremental plans keep their add/update/remove counts without inventing an exact transfer-byte value. The existing guarded CLI lifecycle remains the only production-write path. Credentials, private keys, server IPs, and raw SSH configuration remain outside metadata-editor.",
  },
] as const;
...
export const workflowFaqItems: readonly WorkflowFaqItem[] = [
  {
    ...
    answer:
      "Use the Hiplingo logo at the far left of the application header to return to the Library landing workspace. Selecting a release from Library opens a browsing-oriented Release overview inside the Library workspace; detailed metadata inspection is entered only through Edit metadata, or from Ingest/Staging workflows that intentionally open the editor. When a Library release is open, click Library in the workflow navigation to return to the Library browser context that opened it, including the current Releases/Artists scope and its in-session display mode; the redundant release-detail back arrow is intentionally omitted. The contextual Back to editor control inside Workflow & Help still returns to the workspace or release detail that opened Help.",
  },
  {
    question: "What does the Library Tiles tooltip show?",
    answer:
      "Tiles is the Artwork-First Library default on every fresh browser load or reload. Rows, Cards, and Waveform remain available for the current session, but the selected Library view is not persisted across refreshes. Rows and Cards keep their existing visible provenance and health information. Tiles keeps the album artwork clear at rest; hover a tile or move keyboard focus into it to reveal release title, artist, date, track/video counts, canonical Library and artwork source paths, and the same file-spec, technical-health, metadata-readiness, and Library-health badges used by the other Library views. While Tiles is active, the Library toolbar also shows a Tile size slider with finer 0.25rem steps so the artwork grid can be made denser or larger without changing Rows or Cards. Tile size starts at 16rem on each browser load and resets on refresh instead of being persisted. Waveform view is a listening-oriented Now Playing surface rather than a second Library browser or playback engine: it uses the persistent application-shell player as the single current-track authority and renders that same track through the shared Hiplingo visualization surface with fixed-center mouse/touch scrubbing, brief audible scrub auditions, waveform zoom, and oscilloscope stages. Release and track selection belong to Rows, Cards, Tiles, Artist-filtered Library views, and Release overview pages. Changing the persistent player track immediately changes the large Waveform view, so the footer waveform and Library Waveform view cannot represent different tracks. The shared Web Audio media/analyser graph remains initialized before normal playback, preserving responsive audible scrubbing.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "Use Web Package → Make Public after Ready Check passes and web derivatives are current. Ready Check also owns the release's public-media selection: all canonical tracks remain private Library members, while a reviewed subset can be exposed in the Web Package; public video is a separate per-release toggle that defaults OFF for the v1.0.0 audio-focused package. The writer copies only those reviewed player-facing resources into a staged snapshot, validates them, and atomically promotes the result; it never moves the canonical release. Update Web Package replaces that complete release snapshot, so deselected tracks and disabled video lose their old public files and URLs instead of lingering. Later metadata, artwork, or revised-master changes remain authored in media-library and are republished with Update Web Package.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "Media jobs stay explicit rather than running during ordinary metadata saves. Library waveform generation is part of the explicitly confirmed Staging Build because waveform-peaks.wfp is required for immediate Library presentation; private playback-MP3 and HLS transcoding remain separate reviewed preparation actions.",
  },
  {
...
export const workflowTroubleshootingItems: readonly WorkflowTroubleshootingItem[] = [
  ...
  },
  {
    ...
    description:
      "Incremental updates require a valid ingest-receipt.json. When an existing canonical Library release predates receipts, Staging now exposes one guarded Migrate legacy receipt action before update preview. The migration fingerprints the current canonical masters, videos, and authored artwork, requires the reviewed fingerprint to remain current, and writes only the missing receipt; it does not rewrite the media or metadata it baselines. After migration, return to Artwork & files and explicitly confirm any occupied artwork replacement before previewing the update.",
  },
  {
